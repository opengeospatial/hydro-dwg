...
\<img alt="[BoundaryProblem](BoundaryProblem.md)" height="212" src="<https://raw.githubusercontent.com/opengeospatial/hydro-dwg-wiki-docs/main/Gwml2ConsolFeatureListDiscussionPage/image2.png>" width="319" /\>
...
[HydrologicRole](HydrologicRole.md) is a very interesting conception, but in my understanding "role" is rather a property, not a feature. Feature can play more then one role also at the same time.

By the way: Association class cannot be implemented in XML directly. It must be substituted by kind proxy (broker) class, as it is often done in [GeoSciML](GeoSciML.md).

I propose to return to „traditional” conception as it is on image below:

\<img alt="[HydrogeologicUnit](HydrogeologicUnit.md)" height="432" src="<https://raw.githubusercontent.com/opengeospatial/hydro-dwg-wiki-docs/main/Gwml2ConsolFeatureListDiscussionPage/HydrogeologicUnit.png>" width="595" /\>
...
You are right, my diagram is incorrect. I should have used **[HydrogeologicUnit](HydrogeologicUnit.md)**. In GWML , [HydrostratigraphicUnit](HydrostratigraphicUnit.md) is a subtype of [HydrogeologicUnit](HydrogeologicUnit.md) (which is a subtype of [GeologicUnit](GeologicUnit.md))

\> "Hydrogeological Unit (or Hydrostratigraphic Unit) is not a subtype of Geologic Unit (inheritance relation)"

I think this is the core of the discussion to have here. We don't understand that inheritance relation the same way. My interpretation is : Inheritance relation does not mean that [HydrogeologicUnit](HydrogeologicUnit.md) is bounded by [GeologicUnit](GeologicUnit.md). It just says it's also a package of rock that is bounded differently.
...
I see we should return to our discussion (stopped few months ago) about relation between GWML and [GeoSciML](GeoSciML.md). More strictly, relation between [HydrogeologicUnit](HydrogeologicUnit.md) and [GeologicUnit](GeologicUnit.md).
...
##### **\<a name="Topic10"\>Topic 10:** **Relation between GWML and [GeoSciML](GeoSciML.md) (more strictly, relation between [HydrogeologicUnit](HydrogeologicUnit.md) and [GeologicUnit](GeologicUnit.md))**

In [GeoSciML](GeoSciML.md) [GeologicUnit](GeologicUnit.md) is a parent class for all underground units: **[GeologicUnit](GeologicUnit.md)**, [AllostratigraphicUnit](AllostratigraphicUnit.md), [AlterationUnit](AlterationUnit.md), [ArtificialGround](ArtificialGround.md), [BiostratigraphicUnit](BiostratigraphicUnit.md), [ChronostratigraphicUnit](ChronostratigraphicUnit.md), [DeformationUnit](DeformationUnit.md), [ExcavationUnit](ExcavationUnit.md), **[GeophysicalUnit](GeophysicalUnit.md)**, [LithodemicUnit](LithodemicUnit.md), [LithogeneticUnit](LithogeneticUnit.md), [LithologicUnit](LithologicUnit.md), [LithostratigraphicUnit](LithostratigraphicUnit.md), [LithotectonicUnit](LithotectonicUnit.md), [MagnetostratigraphicUnit](MagnetostratigraphicUnit.md), [MassMovementUnit](MassMovementUnit.md), Pedoderm, [PedostratigraphicUnit](PedostratigraphicUnit.md), [PolarityChronostratigraphicUnit](PolarityChronostratigraphicUnit.md), also **in GWML: [HydrogeologicUnit](HydrogeologicUnit.md)**.In consequence every object of type [HydrogeologicUnit](HydrogeologicUnit.md) (as an instance of class) is also of (sub-) type [GeologicUnit](GeologicUnit.md). In this case object of type [HydrogeologicUnit](HydrogeologicUnit.md) we can see as an object of type [GeologicUnit](GeologicUnit.md) with attribute “geologicUnitType = [HydrogeologicUnit](HydrogeologicUnit.md) ” and with additional hydrogeological properties.Class [GeologicUnit](GeologicUnit.md) is not abstract, it can be instantiated. So direct object of this type can exist (with attribute “geologicUnitType = [GeologicUnit](GeologicUnit.md) ”. In this case boundaries of object of type [HydrogeologicUnit](HydrogeologicUnit.md) can be different then of type [GeologicUnit](GeologicUnit.md). We have two types of object: with attribute “geologicUnitType = [HydrogeologicUnit](HydrogeologicUnit.md) ” and with attribute “geologicUnitType = [GeologicUnit](GeologicUnit.md) ".I think it is inconsistency concerning the [GeologicUnit](GeologicUnit.md) concept (and class) in [GeoSciML](GeoSciML.md) and we must deal with it in GWML. I see the same problem with [GeophysicalUnit](GeophysicalUnit.md) but in INSPIRE model for subtheme Geophysics is independent from subtheme Geology and in consequence from [GeoSciML](GeoSciML.md).This inheritance has also inconveniences in practical application: overloading by properties not needed for hydrogeology (quotation from [EricBoisvert](EricBoisvert.md) e-mail):
...
\> In this case boundaries of object of type [HydrogeologicUnit](HydrogeologicUnit.md) can be different then of type [GeologicUnit](GeologicUnit.md). We have two types of object: with attribute “geologicUnitType = [HydrogeologicUnit](HydrogeologicUnit.md) ” and with attribute “geologicUnitType = [GeologicUnit](GeologicUnit.md) ".

They are two different units (subdividing the same stack of rock). To be honest, I don't understand why "[GeologicUnit](GeologicUnit.md)" is in the enum. It probably means "unclassified" or "generic". What I would expect to see is a [HydrogeologicUnit](HydrogeologicUnit.md) geologicUnitType="[HydrogeologicUnit](HydrogeologicUnit.md)" (at this point it seems redundant because we only have 1 subtype, but we could have more) and a [GeologicUnit](GeologicUnit.md) geologicUnitType="[LithostratigraphicUnit](LithostratigraphicUnit.md)". They are two distinct units (or to be more precise, two distinct way to subdivide the same stack of rock).

As for unwanted property - you are right. At the last [GeoSciML](GeoSciML.md) meeting it has been recognised as being a problem and, we started working on a way to better package [GeoSciML](GeoSciML.md) to avoid imposing those properties if they are not wanted. [GeoSciML](GeoSciML.md) 4.0 (the one that will be proposed to OGC) decouples the properties from the main class, so a barebone [GeologicUnit](GeologicUnit.md) looks like
...
[GeoSciML4](GeoSciML4.md) - it is very good news. Now, [GeologicUnit](GeologicUnit.md) looks very similar to \_GeologicFeature\_ class. I see it is going in good direction. (I hope we will have “[EarthUnit](EarthUnit.md)” or “[GeoscientificUnit](GeoscientificUnit.md)” in this palace in [GeoSciML5](GeoSciML5.md).)

\_\>To be honest, I don't understand why "[GeologicUnit](GeologicUnit.md)" is in the enum.\_

It is consistent with fact that [GeologicUnit](GeologicUnit.md) is not abstract so can be instantiated. I think it was designed as a pure geologic unit on the beginning and was extended later as a parent class for many other types of unit in scope of geosciences. I hope it will be amended in new version.

In that case, should we direct our works on GWML2 to harmonization with [GeoSciML4](GeoSciML4.md)?

\_\>[HydrogeologicUnit](HydrogeologicUnit.md) geologicUnitType="[HydrogeologicUnit](HydrogeologicUnit.md)" (...) and a [GeologicUnit](GeologicUnit.md) geologicUnitType="[LithostratigraphicUnit](LithostratigraphicUnit.md)". They are two distinct units (or to be more precise, two distinct way to subdivide the same stack of rock).\_

I agree with it but we still have [GeologicUnit](GeologicUnit.md) geologicUnitType="[GeologicUnit](GeologicUnit.md)".

-- JanuszMichalak - 08 Feb 2013

------------------------------------------------------------------------

\_\>It is consistent with fact that [GeologicUnit](GeologicUnit.md) is not abstract so can be instantiated. I think it was designed as a pure geologic unit on the beginning and was extended later as a parent class for many other types of unit in scope of geosciences. I hope it will be amended in new version.\_

[GeologicUniType](GeologicUniType.md) is just a way to "softype" since all geologic unit types (lithostrat, lithodemic, etc.) are just packages of rock whatever the classification system (for instance, a biostratigraphic unit can have a lithology, even if it's not the defining character of a biostratigraphic unit). So it did not make sense to create the subtypes if they did not have any specific properties (some might argue otherwise - Boyan ?). It also allows management of "informal" units. Maybe at the end, [HydrologicUnit](HydrologicUnit.md) would not even need a hard subtype and just an addition to the list of geologicUnitType.

\_\>I agree with it but we still have [GeologicUnit](GeologicUnit.md) geologicUnitType="[GeologicUnit](GeologicUnit.md)".\_

Yeah - it should not be there. it should be "[InformalGeologicUnit](InformalGeologicUnit.md)" or something like this.

-- EricBoisvert - 08 Feb 2013

------------------------------------------------------------------------

*\>It also allows management of "informal" units.*What exactly means „[InformalGeologicUnit](InformalGeologicUnit.md)”? It is not clear for me. Should [GeologicUnit](GeologicUnit.md) be still non-abstract (instantiated)? What version of [GeoSciML](GeoSciML.md) will be base for GWML3?
...
[GeologicUnit](GeologicUnit.md) is still concrete.

\>What version of [GeoSciML](GeoSciML.md) will be base for GWML3?

[GeoSciML](GeoSciML.md) 4, since [GeoSciML](GeoSciML.md) 4 will be an OGC standard and we want to make a OGC standard, it make sense to use v4.
...
Can we know more about [GeoSciML4](GeoSciML4.md)? For example, time table of development or initial assumptions.
...
About [GeoSciML](GeoSciML.md) 4: see <http://www.ogcnetwork.net/node/212>
