...
From my experience with [GeoSciML](GeoSciML.md), devil is in the details and detailed Use Cases really saves a lot of discussion. This is just a draft document we can start to polish as we agree on the interoperability scenario.

## CHARACTERISTIC INFORMATION

CASE: serialise [WaterML](WaterML.md) 2.0 O&M groundwater levels around the Lake superior bassin using data from various sources in USA and Canada. The services must conform to SOS 1.0 (OGC 06-009r6) and support the mandatory methods described in SOS 1.0 specification (<http://portal.opengeospatial.org/modules/admin/license_agreement.php?suppressHeaders=0&access_license_id=3&target=http://portal.opengeospatial.org/files/index.php?artifact_id=26667>).

### Goal in Context

Test the [WaterML](WaterML.md) 2.0 model and GML encoding to deliver groundwater level data
...
O&M (OGC 07-022r1) Sampling (OGC 07-002r3) [WaterML](WaterML.md) 2.0 GWML 1.0 [SensorML](SensorML.md) (07-000 / 07-122r2) GML 3.1.1
...
- SOS 1.0 service that generates [WaterML](WaterML.md) 2.0
...
- [WaterML](WaterML.md) schemas is network accessible (for validation)
- Provider have a WMS layer for their sensor network

**Success End Condition:**

- User can get a valid Capabilities document
- User can get a valid [DescribeSensor](DescribeSensor.md) document
...
- User uses the query tool to click to click on one point to extract Observation at this site. (see Issue 1 in Open Issues)
  ...
  - User clicks on Sensor link that returns a [SensorML](SensorML.md) document about the sensor.
- User zooms to a given area
  - User clicks on 'download' button to get all the Observation in the current [AoI](AoI.md)
...
- [GetCapabilities](GetCapabilities.md) : GetCapabilities details
- [GetObservationAtLocation](GetObservationAtLocation.md) : Get information at a location (single point) when user clicks on a dot.
