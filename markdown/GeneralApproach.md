...
I also like what NOAA has up here as an example. It takes POST requests. I'd propose similar output to the [WaterLevel](WaterLevel) and [WaterLevel](WaterLevel) (timeseries) but via REST: <http://sdf.ndbc.noaa.gov/sos/test.shtml>
...
We'd also look at a [WaterML](WaterML) or GWML encoding vs the IOOS DIF as is used in <http://sdf.ndbc.noaa.gov/sos/test.shtml>
...
[GetObservation](GetObservation) for a Time Range and a Specific time, as well as BBOX are covered in the spec, I'm not sure how we'll do the 'latest' use case. Will check the spec.
...
To expose [WaterLevel](WaterLevel), we need to name those property through ideous URN. We'll probably need to craft one of those for [WaterLevel](WaterLevel)
...
It isn't possible to request for particular [UoM](UoM) currently unless you define your observedProperty to include a [UoM](UoM) (e.g. [WaterCourseLevel_m](WaterCourseLevel_m)) but this is probably not the best way to do it.
