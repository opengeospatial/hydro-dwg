...
2.  If not already done, do a SVN checkout for the following packages
    1.  [GeoSciML](GeoSciML) --\> <https://www.seegrid.csiro.au/subversion/GeoSciML/tags/3.2.0/model>
    2.  [HollowWorld](HollowWorld) --\> <https://www.seegrid.csiro.au/subversion/HollowWorld/branches/release_4>
...
3.  In EA, create a new empty project. In Project viewer, right-click on model, --\> Version control --\> then Version control Setting. Create new configurations by clicking on the new button.
    ...
    4.  Type geosciml in the [UniqueID](UniqueID) field. Select the path where the [GeoSciML](GeoSciML) model resides. Save and click on the New button.
    5.  Type gwml in the [UniqueID](UniqueID) field. Select the path where the gwml model resides (typically, \<gwml_root\>\model). Save.
4.  In EA, in Project viewer, right-click on model, --\> Package control --\> Get Package. For [HollowWorld](HollowWorld) (select file [HollowWorld](HollowWorld).xml). Do the same for [GeoSciML](GeoSciML) (select file geosciml.xml), For GWML, select GWML2.xml. If you placed your ISO TC211 models files correctly, the get package for isotc211 is done automatically through [HollowWorld](HollowWorld)
...
7.  Add the Inspire profile (for voidable properties) in the UML Profiles in the Resources pane. Select the file [INSPIRE_UMLProfile_D2](INSPIRE_UMLProfile_D2).5_v3.0.xml (from the hollowWorld location). Same procedure than the previous point.
...
Updated -- [FrancoisLetourneau](FrancoisLetourneau) - 05 Feb 2015

## Troubleshooting

**EA throws a [QueryDef](QueryDef) error when you try to Get Latest or Get All Latest.**
