# [OM_Observation](OM_Observation) used as property values

There are a couple of things that needs to be set in the way we encode [OM_Observation](OM_Observation), specially when we use them as property values.

There are two cases when we use [OM_Observation](OM_Observation) as values

1.  When using relatedObservation on any sub types of [SF_SamplingFeature](SF_SamplingFeature)
2.  Using a specific property (such as [GW_FluidBody](GW_FluidBody)/gwBodyVolume)

[OM_Observation](OM_Observation) has a mandatory featureOfInterest that is supposed to point back to the feature that is the “bearer of the property”. In case 2) , the bearer of the property is obviously the feature having this property, so technically, we should constrain featureOfInterest to link back to that feature
...
([GW_WaterBudget](GW_WaterBudget) for instance, is a Type, not a Feature), so we can’t use featureOfInterest to link it back

In this case, we should point back to the refers to this Type ? In case of [WaterBudget](WaterBudget), who does ?

## Same Observation shared by multiple properties

nothing prevents more than one property to point to the same Observation (by reference), which becomes a problem for the featureOfInterest (foi is 1..1). It can be done schematically even if does not make sense logically. Should we have a clause that says “[OM_Observation](OM_Observation) that are values of explicit properties will use featureOfInterest to identify the legitimate bearer of properties. Reference to [OM_Observation](OM_Observation) from an explicit property when the property bearer is not the owner is invalid” (should it be invalid ?, or can it have another meaning)

## [ObservedProperty](ObservedProperty)

[OM_Observation](OM_Observation) also has an observedProperty. Technically, this should be a reference to the property (the property type, not a specific instance of the property) this observation is about. We have several options (which one should we use)
